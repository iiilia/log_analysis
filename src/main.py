    ...
    for key in list(time_type_structure.keys()):
        ...
        if in_second_structure.get('count_in', 0) == in_second_structure.get('count_in', 0) \
                ...
                and time.strftime(key[0], '%h:%m:%s') == (time.strftime(last_time_write, '%h:%m:%s') + \
                                                                  timedelta(seconds=1)):
            # TODO это условие нужно переписать нормально (ПРОВЕРИТЬ)
            result_line = count_stat_by_second(in_second_structure, key)
            f_out.write(result_line)
            # TODO удалить по ключу (время, тип) из словаря

def count_stat_by_second(in_second_structure, info):
    res = list()
    # считаем статистику
    res.append(info[0])
    res.append(info[1])
    res.append(in_second_structure['count_in'])
    latency = []
    for key_ in list(in_second_structure.keys()):
        if key_ not in [u'count_in', u'count_out', u'now_or_past']:
            # TODO  проверить что говно снизу возвращает миллисекунды
            latency.append(in_second_structure[key_]['a'] - in_second_structure[key_]['q'])
    res.append(np.mean(latency))
    res.append(np.median(latency))
    res.append(np.percentile(latency, 90))
    res.append(np.percentile(latency, 99))
    res.append(max(latency))
...
def line_processing(line):
    ...
    if len(stop_regex.findall(line)) > 0:
        ...
        id = line[0]
        # TODO получение типа сообщения
        type_of_line = u''
        # TODO запрос или ответ???
        q_or_a = u''
...
def __main__(*argv):
    if argv and argv[0] > 2:
        ...
        # TODO путь к файлу с результатами
        path_to_result = u''
